...
## Key Components
- **Markdown Files**: `drinks.md` (detailed recipes), `menu.md` (guest menu), `coquito.md` (special recipe)
...
- **Conversion Logic**: Scripts loop over file pairs like `('drinks.md', 'drinks.docx')` using `pypandoc.convert_file()`
- **Error Handling**: Basic try/except in scripts; assume pandoc is available
- **Updates**: When editing recipes, update md first, then convert to docx for sharing
- **Reference Files**: See `convert_to_word.py` for conversion implementation, `menu.md` for menu format
